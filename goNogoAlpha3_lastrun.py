...
expName = 'goNogoAlpha3'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Andrew\\Documents\\Work\\Projects\\goNogoAlpha3\\goNogoAlpha3_lastrun.py',
...
for thisTask in tasks:
    ...
    for thisTrial in trials:
        ...
        trialText.text = str(colorChoose)
        trialCount.text = str(trials.thisN+1)
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if trialCircle.status == STARTED:
                ...
                if tThisFlipGlobal > trialCircle.tStartRefresh + 0.75-frameTolerance:
            ...
            if trialResp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > trialResp.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if trialText.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > trialText.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if trialCount.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > trialCount.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
